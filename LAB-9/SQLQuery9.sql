...
--9.Create view for the student whose name starts with M and ends with a, having SPI more than 6 and backlog less than 2.
    CREATE VIEW STUDENT1 AS SELECT * FROM STUDENT_INFO WHERE NAME LIKE 'M%a' AND SPI > 6 AND BKLOG < 2
    SELECT * FROM STUDENT1
--10.Create the view for the students whose name contains vowel.
    CREATE VIEW STUDENT2 AS SELECT * FROM STUDENT_INFO WHERE NAME LIKE '%[A,E,I,O,U]%'
    SELECT * FROM STUDENT2

--Part – B:
--1.Create a view that displays information of all students whose SPI is above 8.5
    CREATE VIEW DISPLAY_INFORMATION AS SELECT * FROM STUDENT_INFO WHERE SPI > 8.5
    SELECT * FROM DISPLAY_INFORMATION
--2.Create a view that displays 0 backlog students.
    CREATE VIEW DISPLAY_STUDENT AS SELECT * FROM STUDENT_INFO WHERE BKLOG = 0
